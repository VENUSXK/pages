      ...
      <div className="space-y-4 px-6 py-7">
        {data.map((item) => (
          <div key={item.label} className="grid grid-cols-[minmax(0,1fr)_5.75rem] items-center gap-3">
            <div
              className="relative h-10 overflow-hidden rounded-none bg-fd-muted/70"
              role="img"
            ...
            >
              ...
              <div
                aria-hidden="true"
                className={`absolute inset-y-0 left-0 min-w-1 rounded-none ${item.label === accent ? 'bg-stone-400 dark:bg-stone-600' : 'bg-stone-200 dark:bg-stone-800'}`}
